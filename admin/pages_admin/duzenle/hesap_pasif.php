...
if(@$hesap->firma_id == $firma_id OR $cikti->uye_yetki == 1 ) { 
 ...
 $firmalar = $conn->prepare("SELECT * FROM hesapbilgileri WHERE id = ?");
 ...
 if ($firmalar) {
     $firma = $firmalar->fetch(PDO::FETCH_OBJ);
      ...
      $firma_duzenle = $conn->prepare("UPDATE hesapbilgileri SET hesapdurum = ? WHERE id = ? ");
  ...
  if ($firma_duzenle) {
        ...
        echo '
                     ...
                     header("Location: uye_hesap_duzenle.php?id=$uyeid");
